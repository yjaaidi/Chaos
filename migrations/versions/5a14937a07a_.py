...
def downgrade():
    ...
    op.execute("DELETE FROM associate_impact_pt_object WHERE pt_object_id IN (SELECT id FROM pt_object WHERE  type='line_section')")
    op.execute("DELETE FROM pt_object WHERE  type='line_section'")
