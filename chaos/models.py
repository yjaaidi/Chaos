...
class Disruption(TimestampMixin, db.Model):
    ...
    @classmethod
    def generate_disruption_search_query_args(cls, filter):
        if 'current_time' not in filter: filter['current_time'] = get_current_time()
        ...
        vars['contributor_id'] = filter['contributor_id']
        bindparams['contributor_id'] = db.String

        andwheres.append('d.status IN :disruption_status')
        vars['disruption_status'] = tuple(filter['statuses'])
        ...
        if isinstance(filter['uri'], basestring) and filter['uri']:
            uri_filters = []
            uri_filters.append('po.uri = :uri')
            bindparams['uri'] = db.String
            vars['uri'] = filter['uri']
            if filter['line_section']:
                ...
                vars['uri_like'] = filter['uri'] + ':%'
            andwheres.append('(' + ' OR '.join(uri_filters) + ')')
        elif filter['ptObjectFilter'] is not None:
            ptObjectIds = [id for ids in filter['ptObjectFilter'].itervalues() for id in ids]
            if ptObjectIds:
                uri_filters = []
                ...
                if filter['line_section'] and 'lines' in filter['ptObjectFilter'] and filter['ptObjectFilter']['lines']:
                    ...
                    vars['po_type_line_section'] = 'line_section'
                    vars['po_line_section_lines'] = tuple(filter['ptObjectFilter']['lines'])
                andwheres.append('(' + ' OR '.join(uri_filters) + ')')

        if isinstance(filter['cause_category_id'], basestring) and filter['cause_category_id']:
            andwheres.append('c.category_id = :cause_category_id')
            bindparams['cause_category_id'] = db.String
            vars['cause_category_id'] = filter['cause_category_id']

        if isinstance(filter['tags'], list) and filter['tags']:
            andwheres.append('t.id IN :tag_ids')
            bindparams['tag_ids'] = db.String
            vars['tag_ids'] = tuple(filter['tags'])

        if filter['ends_after_date']:
            andwheres.append('d.end_publication_date >=:ends_after_date')
            bindparams['ends_after_date'] = db.Date
            vars['ends_after_date'] = filter['ends_after_date']

        if filter['ends_before_date']:
            andwheres.append('d.end_publication_date <=:ends_before_date')
            bindparams['ends_before_date'] = db.Date
            vars['ends_before_date'] = filter['ends_before_date']

        application_status = set(filter['application_status'])
        if len(application_status) != len(application_status_values):
            application_availlable_filters = {
            ...
            vars['current_time'] = filter['current_time']

        publication_status = set(filter['publication_status'])
        if len(publication_status) != len(publication_status_values):
            publication_availlable_filters = {
        ...
        if isinstance(filter['application_period'], dict) and 'begin' in filter['application_period'] and 'end' in filter['application_period']:
            app_period_filters = []
    ...
    @classmethod
    def count_all_with_post_filter(cls, filter):
        if uri_is_not_in_pt_object_filter(uri=filter['uri'], pt_object_filter=filter['ptObjectFilter']):
            return 0

        query_parts = cls.generate_disruption_search_query_args(filter)
    ...
    @classmethod
    def get_disruption_ids_with_post_filter_native(cls, filter):
        query_parts = cls.generate_disruption_search_query_args(filter)
        ...
        offset = (max(1, filter['page_index']) - 1) * filter['items_per_page']
        vars['limit'] = filter['items_per_page']
        vars['offset'] = offset
    ...
    @classmethod
    def all_with_post_filter_native(cls, filter):

        disruption_ids = cls.get_disruption_ids_with_post_filter_native(filter)
...
class Impact(TimestampMixin, db.Model):
    ...
    @classmethod
    def generate_impacts_search_query_args(cls, filter):
        if 'current_time' not in filter: filter['current_time'] = get_current_time()
        andwheres = []
        vars = {}
        bindparams = {}
        ...
        if filter['ptObjectFilter'] is not None:
            ptObjectIds = [id for ids in filter['ptObjectFilter'].itervalues() for id in ids]
            if ptObjectIds :
                uri_filters = []
                uri_filters.append('po.uri IN :pt_objects_uris')
                ...
                if 'lines' in filter['ptObjectFilter'] and filter['ptObjectFilter']['lines']:
                    uri_filters.append('(po.type = :po_type_line_section AND po_line.uri IN :po_line_section_lines)')
                    bindparams['po_type_line_section'] = db.String
                    bindparams['po_line_section_lines'] = db.String
                    vars['po_type_line_section'] = 'line_section'
    ...
    def count_all_with_post_filter(cls, filter):
        ...
        query_parts = cls.generate_impacts_search_query_args(filter)
    ...
    @classmethod
    def get_impact_ids_with_post_filter_native(cls, filter):
        query_parts = cls.generate_impacts_search_query_args(filter)
    ...
    def all_with_post_filter_native(cls, filter):
        ...
        impact_ids = cls.get_impact_ids_with_post_filter_native(filter)
